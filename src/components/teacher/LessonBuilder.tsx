...
const LessonBuilder: React.FC<LessonBuilderProps> = ({ grades, onSave, onCancel, subject = "math", initialData }) => {
  ...
  const [contentBlocks, setContentBlocks] = useState<LessonContent[]>(initialData?.content || [initialContent]);
  ...
  const [blockPrompts, setBlockPrompts] = useState<{ [key: string]: string }>({});
  ...
  const handleGenerateBlockContent = async (index: number) => {
    const block = contentBlocks[index];
    const prompt = blockPrompts[block.id];
...
Task: ${prompt}`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const lesson: Lesson = {
      ...
      learningType: selectedLearningType || initialData?.learningType || 'custom',
  ...
  return (
    <div className="w-full">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">{initialData ? 'Edit Lesson' : 'Create New Lesson'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-8">
                      ...
                      {block.type === 'text' && (
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground bg-purple-50 px-2 py-1 rounded">
                          ...
                          {/* AI Prompt Section */}
                          <div className="flex gap-2 items-center p-3 bg-slate-50 rounded-md border border-slate-100">
                            <Wand2 size={16} className="text-purple-600 flex-shrink-0" />
                            <Input
                              placeholder="Ask AI to write this section (e.g., 'Explain addition to grade 1 students')"
                              className="flex-1 border-slate-200 bg-white h-9 text-sm"
                              value={blockPrompts[block.id] || ''}
                              onChange={(e) => setBlockPrompts(prev => ({ ...prev, [block.id]: e.target.value }))}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  e.preventDefault();
                                  handleGenerateBlockContent(index);
                                }
                              }}
                            />
                            <Button
                              type="button"
                              size="sm"
                              onClick={() => handleGenerateBlockContent(index)}
          ...
          <div className="flex justify-between items-center pt-4">
            ...
            <Button type="submit">
              {initialData ? 'Save Changes' : 'Save Lesson'}
            </Button>
          </div>
        </div>
      </form>
