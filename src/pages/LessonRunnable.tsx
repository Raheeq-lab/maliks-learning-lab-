...
const LessonRunnable: React.FC = () => {
    ...
    const [isTeacherMode, setIsTeacherMode] = useState(false); // Teacher Mode State
    ...
    // Updated Phase Colors with Specific Palette
    const getPhaseColorDetails = (phase: keyof LessonStructure) => {
        switch (phase) {
            ...
            case "guidedPractice": return {
                bg: "bg-[#45B7D1]", // Blue
                text: "text-white",
                lightBg: "bg-[#45B7D1]/10",
                border: "border-[#45B7D1]",
            ...
            case "independentPractice": return {
                bg: "bg-[#96CEB4]", // Mint
                text: "text-white",
                lightBg: "bg-[#96CEB4]/10",
                border: "border-[#96CEB4]",
                iconColor: "text-[#96CEB4]",
                label: "Try It Yourself"
            };
            case "reflect": return {
                bg: "bg-[#FECA57]", // Yellow
                text: "text-black",
                lightBg: "bg-[#FECA57]/10",
                border: "border-[#FECA57]",
                iconColor: "text-[#FECA57]",
    ...
    return (
        <div className="min-h-screen bg-bg-primary p-6 transition-colors duration-300">
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <Button
                            ...
                            className={`gap-2 ${isTeacherMode ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'text-text-secondary hover:text-text-primary'}`}
                        >
                            {isTeacherMode ? <Layout size={18} /> : <BookOpen size={18} />}
                            {isTeacherMode ? "Teacher Dashboard" : "Teacher Mode"}
                        </Button>
                ...
                {/* Dual View Layout */}
                <div className={`grid ${isTeacherMode ? 'grid-cols-12 gap-6' : 'grid-cols-1'}`}>
                    {isTeacherMode && (
                        <div className="col-span-4 h-[calc(100vh-140px)] sticky top-24 overflow-y-auto pr-2 custom-scrollbar space-y-4">
                            {/* TEACHER CONTROL PANEL */}
                            <Card className="border-2 border-indigo-500/30 bg-indigo-50/50 shadow-sm border-dashed">
                                <CardHeader className="pb-3 pt-4 px-4">
                                    <CardTitle className="text-xs font-bold uppercase tracking-wider text-indigo-700 flex items-center gap-2">
                                        <Zap size={14} className="text-indigo-600" /> Teacher Control Center
                                    </CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-6 px-4 pb-4">
                                    {/* Timer Controls */}
                                    <div className="space-y-2">
                                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Phase Timer Control</span>
                                        <div className="flex gap-2">
                                            <Button
                                                onClick={handleToggleTimer}
                                                size="sm"
                                                className={`flex-1 h-9 text-xs font-bold ${isRunning ? "bg-amber-500 hover:bg-amber-600" : "bg-green-600 hover:bg-green-700"}`}
                                            >
                                                {isRunning ? <><Pause size={14} className="mr-1.5" /> PAUSE</> : <><Play size={14} className="mr-1.5" /> START PHASE</>}
                                            </Button>
                                            <Button onClick={handleResetTimer} size="sm" variant="outline" className="h-9 w-9 p-0 bg-white border-gray-200"><RotateCcw size={14} /></Button>
                                        </div>
                                    </div>

                                    {/* Talking Points */}
                                    {lesson.lessonStructure?.[PHASES[currentPhaseIndex]]?.visualMetadata?.talkingPoints && (
                                        <div className="space-y-2">
                                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
                                                <MessageSquare size={12} /> Key Talking Points
                                            </span>
                                            <ul className="list-disc list-outside ml-3 text-xs text-indigo-900 bg-white p-3 rounded-lg border border-indigo-100 shadow-sm space-y-1">
                                                {lesson.lessonStructure[PHASES[currentPhaseIndex]].visualMetadata.talkingPoints.map((tp: string, i: number) => (
                                                    <li key={i} className="pl-1">{tp}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    )}
                                    ...
                                    {/* Misconceptions */}
                                    {lesson.lessonStructure?.[PHASES[currentPhaseIndex]]?.visualMetadata?.misconceptionAlert && (
                                        <div className="p-3 bg-red-50 border border-red-100 rounded-lg shadow-sm">
                                            <span className="text-[10px] font-bold text-red-600 uppercase tracking-wider flex items-center gap-1.5 mb-1.5">
                                                <AlertCircle size={12} /> Misconception Alert
                                            </span>
                                    ...
                                    {/* Differentiation */}
                                    {(lesson.lessonStructure?.[PHASES[currentPhaseIndex]]?.visualMetadata?.differentiationLow || lesson.lessonStructure?.[PHASES[currentPhaseIndex]]?.visualMetadata?.differentiationHigh) && (
                                        <div className="space-y-2">
                                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
                                                <BrainCircuit size={12} /> Differentiation
                                            </span>
                                            <div className="space-y-2">
                                                {lesson.lessonStructure[PHASES[currentPhaseIndex]].visualMetadata.differentiationLow && (
                                                    <div className="text-[11px] bg-green-50 p-2 rounded border border-green-100 text-green-900">
                                                        <strong className="block text-green-700 text-[9px] uppercase mb-0.5">Support:</strong>
                                                        {lesson.lessonStructure[PHASES[currentPhaseIndex]].visualMetadata.differentiationLow}
                                                    </div>
                                                )}
                                                {lesson.lessonStructure[PHASES[currentPhaseIndex]].visualMetadata.differentiationHigh && (
                                                    <div className="text-[11px] bg-purple-50 p-2 rounded border border-purple-100 text-purple-900">
                                                        <strong className="block text-purple-700 text-[9px] uppercase mb-0.5">Challenge:</strong>
                                                        {lesson.lessonStructure[PHASES[currentPhaseIndex]].visualMetadata.differentiationHigh}
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    <div className="space-y-2 pt-4 border-t border-indigo-200/50">
                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Live Class Pulse</span>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="bg-white p-2 rounded border border-gray-100 text-center shadow-sm">
                                                <div className="text-lg font-bold text-success-green">92%</div>
                                                <div className="text-[9px] text-gray-400 uppercase font-bold">Engagement</div>
                                            </div>
                                            <div className="bg-white p-2 rounded border border-gray-100 text-center shadow-sm">
                                                <div className="text-lg font-bold text-focus-blue">4.8</div>
                                                <div className="text-[9px] text-gray-400 uppercase font-bold">Understanding</div>
                                            </div>
                                        </div>
                                    </div>
                                </CardContent>
                    ...
                    <div className={isTeacherMode ? "col-span-8" : "col-span-12"}>
                        {/* Main Lesson Content */}
                        <Card className="border-0 shadow-lg overflow-hidden bg-bg-card rounded-xl h-full flex flex-col">
